...
from .models import (
    User, Products, SlideShow, Carts, Receipt, Order, Category, Location,TableBooking,Table
)
...
from .serializers import LocationSerializer, ReceiptSerializer

# Simple view -> saves location and returns the created object with id
@api_view(['POST'])
@permission_classes([AllowAny])   # set to IsAuthenticated if required
def save_location(request):
    ...
    Response: { "status":200, "message":"Location saved", "data": {"id": <location_id>, ... } }
    """
    serializer = LocationSerializer(data=request.data)
    if serializer.is_valid():
        loc = serializer.save()
        return Response({"status": 200, "message": "Location saved", "data": {"id": loc.id}}, status=201)
    return Response(serializer.errors, status=400)


# Receipt viewset for listing/creating receipts at checkout time
