...
from .models import (
    User, Banner, Products, Carts, Receipt, Order, Category, Location,TableBooking,Table
)
...
from .serializers import (
    ...
    LocationSerializer,TableBookingSerializer,TableSerializer,TableBookingCreateSerializer
)
...
class TableBookingCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "customer_phone",
            "tables",
            "items",        # incoming from Flutter
            "food_list",    # stored in DB
            "total",
            "created_at",
        ]
        read_only_fields = ("food_list", "total", "created_at")

    def create(self, validated_data):
        items = validated_data.pop("items", [])
        tables_ids = validated_data.pop("tables", [])

        # ✅ Create booking first
        booking = TableBooking.objects.create(**validated_data)

        # ✅ Assign tables (by number)
        tables = Table.objects.filter(number__in=tables_ids)
        if tables.count() != len(tables_ids):
            raise serializers.ValidationError(
                "One or more selected tables do not exist"
            )
        booking.tables.set(tables)

        # ✅ Build food_list + calculate total
        food_list = []
        total = Decimal("0.00")
